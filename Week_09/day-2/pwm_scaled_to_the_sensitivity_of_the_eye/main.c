...
float expled(int adc_val)
{

	double exp_adc_val = exp(adc_val / 1000.0);	// convert linear adc_val to log scale

	double rescaled_val = ((exp_adc_val - 1) * (1000 - 0)) / ((60.039 - 1) + 0);// rescale the log value between 0 and 1000
...
int main(void)
{
	...
	while (1) {
		...
		if (HAL_ADC_PollForConversion(&adc_handle, 10) == HAL_OK) {
			...
			printf("ADC value: %d.\n", adc_val);
			__HAL_TIM_SET_COMPARE(&TimHandle, TIM_CHANNEL_1, expled(adc_val));
		}
	}
}
