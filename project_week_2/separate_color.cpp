...
void separateParse(int argc, const char** argv)
{
	...
	case true:
		...
		default:
			std::string error = "Not enough arguments!";
			resultImage = cv::Mat(250, 250, CV_8UC1);
			cv::putText(resultImage, error, cv::Point(50, 125), cv::FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(0, 0, 0), 1);
			break;
		}
		break;
